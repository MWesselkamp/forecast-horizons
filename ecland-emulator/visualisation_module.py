...
class VisualisationModule:
    ...
    def plot_station_data_and_forecast(self, dynamic_features_dict, dynamic_features_prediction_dict, station_data, matching_indices):
        ...
        if 'mlp' in dynamic_features_prediction_dict:
            for i, a in enumerate(ax):
                ax[i].plot(doy_vector,
                           ...
                           color="magenta", label="MLP", alpha = 0.6)
        ...
        for a in ax:
            ...
            a.set_xticks(tick_positions)
            a.set_xticklabels([pd.Timestamp(t).strftime('%Y-%m-%d') for t in tick_positions], rotation=25)
            a.legend(prop=self.legend_properties)
    ...
    def plot_scores(self, scores_l1, scores_l2, scores_3, score, hod = None, log_y=True):
        ...
        for model, scores in scores_l1.items():
            ax[0].plot(doy_vector, scores, color = colors[i], alpha = 0.7, label = model)
            i += 1
        ax[0].set_ylabel(f"{score}", **self.label_properties)

        i = 0
        for model, scores in scores_l2.items():
            ax[1].plot(doy_vector, scores, color = colors[i], alpha = 0.7, label = model)
            i += 1

        i = 0
        for model, scores in scores_3.items():
            ax[2].plot(doy_vector, scores, color = colors[i], alpha = 0.7, label = model)
            i += 1
        ...
        for a in ax:
            a.hlines(0, xmin = min(doy_vector), xmax=max(doy_vector), color = "black", alpha = 0.8, linestyle = '--')
            a.set_xlabel(x_label, **self.label_properties)
            a.legend(prop=self.legend_properties, loc = "upper left")
            if log_y:
                a.set_yscale('log')
            tick_positions = doy_vector[::10]  # Adjust frequency as needed
            a.set_xticks(tick_positions)
    ...
    def plot_horizons(self, scores_l1, scores_l2, scores_3, score, threshold, hod = None, log_y=True):
        ...
        i = 0
        for model, scores in scores_l1.items():
            ax[0].plot(doy_vector, threshold - scores, color = colors[i], alpha = 0.7, label = model)
            i += 1
        ax[0].set_ylabel(f"{score} - $\\varrho$", **self.label_properties)

        i = 0
        for model, scores in scores_l2.items():
            ax[1].plot(doy_vector, threshold - scores, color = colors[i], alpha = 0.7, label = model)
            i += 1

        i = 0
        for model, scores in scores_3.items():
            ax[2].plot(doy_vector, threshold - scores, color = colors[i], alpha = 0.7, label = model)
            i += 1
    ...
    def plot_skill_scores(self, skill_scores_l1, skill_scores_l2, skill_scores_l3, score, hod = None, log_y = True, sharey = True, invert = False):
        ...
        fig, ax = plt.subplots(1, 3, figsize=(15, 5), sharex=True, sharey=sharey) 
